...
/**
 * Routines for reading and writing EncodedData objects to named file. The code in
 * here is what actually touches files on disk.
 */
void writeEncodedData(EncodedData& data, const std::string& filename);
EncodedData readEncodedData(const std::string& filename);


/**
 * Routines for reading and writing raw bytes to named file.
 */
void writeRawBinary(const std::string& data, const std::string& filename);
std::string readRawBinary(const std::string& filename);
